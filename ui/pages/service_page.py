...
class ServicePage(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        rate_button = ctk.CTkButton(
            ...
            command=self.open_cost_settings,
        ...
        rate_button.pack(side="left", padx=(0, 10))
        ...
        form_container = ctk.CTkFrame(main_container, width=400)
        form_container.pack(side="left", fill="both", expand=False, padx=(0, 10))
        ...
        client_search_frame.pack(fill="x", padx=20, pady=(0, 10))
        ...
        self.problem_summary_entry.pack(fill="x", padx=20, pady=(0, 10))
        ...
        self.device_imei_entry.pack(fill="x", padx=20, pady=(0, 10))
        ...
        self.warranty_number_entry.pack(fill="x", padx=20, pady=(0, 10))
        ...
        self.labor_type_segmented = ctk.CTkSegmentedButton(
            labor_type_frame,
        ...
        self.hours_entry.pack(fill="x", padx=20, pady=(0, 10))
        ...
        submit_button.pack(fill="x", padx=20, pady=(0, 20))
        
        # Lado direito: Seleção de componentes (modelo "carrinho de compras")
        components_frame = ctk.CTkFrame(main_container)
        ...
        components_title.pack(pady=(20, 10))
        ...
        scrollable_frame = ctk.CTkScrollableFrame(components_frame, height=220)
        ...
        selected_scrollable = ctk.CTkScrollableFrame(components_frame, height=150)
        selected_scrollable.pack(fill="both", expand=True, padx=20, pady=(0, 20))
    ...
    def refresh_components_list(self, force: bool = False, show_overlay: bool = False):
        ...
        def db_operation():
            ...
            # Filtrar apenas componentes com stock > 0
            components = [c for c in components if c.get("qty", 0) > 0]
            return components
        ...
        def callback(components, error):
            ...
            if auto_select:
                ...
                if len(exact_matches) == 1:
                    ...
                    self.current_selected_component = comp
                    info_text = f"[{comp['code']}] {comp['name']} (Stock Atual: {comp['qty']})"
                    self.selected_component_label.configure(text=info_text)
                    self.component_qty_entry.delete(0, "end")
                    self.component_qty_entry.insert(0, "1")
    ...
    def create_component_item(self, component: dict):
        ...
        item_frame = ctk.CTkFrame(self.components_list_container)
        ...
        details_label = ctk.CTkLabel(
            info_frame,
            text=f"Preço: {component['price']:.2f} € | Stock: {component['qty']}",
            font=ctk.CTkFont(size=12),
            text_color=["#666666", "#999999"],
            anchor="w"
        )
        ...
        def on_select(local_comp=component):
            ...
            self.current_selected_component = local_comp
            info_text = f"[{local_comp['code']}] {local_comp['name']} (Stock Atual: {local_comp['qty']})"
            self.selected_component_label.configure(text=info_text)
            # Focar campo de quantidade
            self.component_qty_entry.delete(0, "end")
            self.component_qty_entry.insert(0, "1")
            self.component_qty_entry.focus_set()
        ...
        # Botão explícito de seleção
        select_button = ctk.CTkButton(
            item_frame,
            text="Selecionar",
            command=lambda lc=component: on_select(lc),
            font=ctk.CTkFont(size=11),
            fg_color=["#4CAF50", "#4CAF50"],
            hover_color=["#45a049", "#45a049"],
            width=90,
    ...
    def add_component_to_repair(self, component: dict, qty_entry):
        ...
        try:
            ...
            if qty > component["qty"]:
                messagebox.showwarning("Atenção", f"Stock insuficiente. Disponível: {component['qty']}")
                return
    ...
    def refresh_selected_list(self):
        ...
        if missing_ids:
            ...
            self.components_cache.update(batch_components)
        
        # Adicionar componentes selecionados
        for comp_id, qty in self.selected_components:
            component = self.components_cache.get(comp_id)
            if component:
    ...
    def create_selected_item(self, component: dict, qty: int):
        ...
        if qty > 1:
            decrease_button = ctk.CTkButton(
                ...
                font=ctk.CTkFont(size=16, weight="bold"),
                fg_color=["#F39C12", "#F39C12"],  # Orange color
    ...
    def open_cost_settings(self):
        ...
        try:
            ...
            # Separador
            separator = ctk.CTkFrame(main_scroll, height=2, fg_color=["#cccccc", "#444444"])
            ...
            rules_container = ctk.CTkScrollableFrame(main_scroll, height=250)
