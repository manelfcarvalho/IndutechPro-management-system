...
from ui.pages.service_page import LaborTypeSelector
from ui.theme import (
    ...
    icon_button_style,
    keep_custom_style,
    payment_status_badge_style,
    repair_status_badge_style,
)
...
class ClientsPage(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        _, buttons_frame = create_page_header(
            self,
            "Historico",
            "Consultar reparacoes, estados, pagamentos e PDFs.",
        )
        ...
        self.status_filter_button = ctk.CTkButton(
            ...
            command=self.open_status_filter_menu,
            **button_style("secondary", compact=True),
            width=160,
        )
        keep_custom_style(self.status_filter_button)
        ...
        search_label.pack(side="left", padx=(14, 10), pady=12)
        ...
        date_filters_frame.pack_forget()

        date_bar = ctk.CTkFrame(
            self,
            fg_color=PANEL_BG,
            corner_radius=8,
            border_width=1,
        ...
        date_bar.pack(anchor="w", padx=20, pady=(0, 10))

        date_preset_row = ctk.CTkFrame(date_bar, fg_color="transparent")
        date_preset_row.pack(side="left", padx=(14, 8), pady=12)

        date_custom_row = ctk.CTkFrame(date_bar, fg_color="transparent")
        date_custom_row.pack(side="left", padx=(0, 14), pady=12)
        ...
        for label, preset in [
            ...
            button = ctk.CTkButton(
                date_preset_row,
                text=label,
                command=lambda value=preset: self.apply_date_preset(value),
                font=ctk.CTkFont(size=12),
                width=78,
                height=30,
                corner_radius=7,
        ...
        self.date_start_entry.pack(side="left", padx=(0, 6))
        ...
        self.date_end_entry.pack(side="left", padx=(0, 6))

        apply_date_button = ctk.CTkButton(
            date_custom_row,
            text="Aplicar",
            command=self.apply_custom_date_filter,
            **button_style("primary", compact=True),
            width=82,
        )
        apply_date_button.pack(side="left", padx=(0, 8))
        ...
        self.date_filter_label.pack(side="left", padx=(0, 0))
        self._update_date_filter_ui("all")
    ...
    def open_status_filter_menu(self):
        """Abre um menu compacto para escolher diretamente o filtro de progresso."""
        existing_menu = getattr(self, "_status_filter_menu", None)
        if existing_menu and existing_menu.winfo_exists():
            existing_menu.destroy()
            self._status_filter_menu = None
            return

        menu = ctk.CTkToplevel(self.app.root)
        self._status_filter_menu = menu
        menu.withdraw()
        menu.overrideredirect(True)
        menu.attributes("-topmost", True)
        menu.configure(fg_color=PANEL_BG)
        ...
        options = [
            ...
            ("Aguardar Pe\u00e7as", "Aguardar Pe\u00e7as"),
            ("Pronto a Entregar", "Pronto a Entregar"),
        ]

        def choose_filter(label, value):
            if menu.winfo_exists():
                menu.destroy()
            self._status_filter_menu = None
            self.current_status_filter = value
            self.status_filter_button.configure(
                text=f"Progresso: {label}",
                **button_style("secondary", compact=True),
            )
            keep_custom_style(self.status_filter_button)
            self.refresh_data(force=True)
        ...
        for index, (label, value) in enumerate(options):
            if value == self.current_status_filter and value != "All":
                style = repair_status_badge_style(value)
            else:
                style = button_style("secondary", compact=True)

            option_button = ctk.CTkButton(
                menu_frame,
                text=label,
                command=lambda l=label, v=value: choose_filter(l, v),
                anchor="w",
                width=172,
                **style,
            )
            keep_custom_style(option_button)
            option_button.pack(
                fill="x",
                padx=6,
                pady=(6 if index == 0 else 2, 6 if index == len(options) - 1 else 2),
            )

        menu.update_idletasks()
        x = self.status_filter_button.winfo_rootx() - menu.winfo_width() + self.status_filter_button.winfo_width()
        y = self.status_filter_button.winfo_rooty() + self.status_filter_button.winfo_height() + 4
        menu.geometry(f"+{max(0, x)}+{max(0, y)}")
        menu.deiconify()
        menu.focus_force()
        menu.bind("<Escape>", lambda _event: menu.destroy())
    ...
    def _set_date_filter(self, start_date, end_date, label_text: str, active_preset: str = "custom"):
        ...
        if hasattr(self, "date_start_entry"):
            ...
            if self.date_filter_start:
                self.date_start_entry.insert(0, self.date_filter_start)

        if hasattr(self, "date_end_entry"):
            self.date_end_entry.delete(0, "end")
            if self.date_filter_end:
                self.date_end_entry.insert(0, self.date_filter_end)

        self._update_date_filter_ui(active_preset)
        self.refresh_data(force=True)

    def _update_date_filter_ui(self, active_preset: str):
        for preset, button in getattr(self, "date_preset_buttons", {}).items():
            is_active = preset == active_preset
            button.configure(
                fg_color="#1a2027" if is_active else "#232a33",
                hover_color="#232a33" if is_active else "#2b3440",
    ...
    def apply_date_preset(self, preset: str):
        ...
        if preset == "all":
            self._set_date_filter(None, None, "Todas as datas", "all")
        elif preset == "today":
            self._set_date_filter(today, today, f"Hoje ({today.strftime('%d/%m/%Y')})", "today")
        elif preset == "last_7":
            start = today - timedelta(days=6)
            self._set_date_filter(start, today, f"Ultimos 7 dias ({start.strftime('%d/%m')} - {today.strftime('%d/%m')})", "last_7")
        elif preset == "last_30":
            start = today - timedelta(days=29)
            self._set_date_filter(start, today, f"Ultimos 30 dias ({start.strftime('%d/%m')} - {today.strftime('%d/%m')})", "last_30")
        elif preset == "month":
            start = today.replace(day=1)
            self._set_date_filter(start, today, f"Este mes ({start.strftime('%d/%m')} - {today.strftime('%d/%m')})", "month")

    def apply_custom_date_filter(self):
        start_text = self.date_start_entry.get().strip()
        end_text = self.date_end_entry.get().strip()

        if not start_text and not end_text:
            self.apply_date_preset("all")
            return
        ...
        try:
            ...
            messagebox.showwarning("Data invalida", "Usa o formato AAAA-MM-DD, por exemplo 2026-06-19.")
            return

        if start_date and end_date:
            label = f"De {start_date.strftime('%d/%m/%Y')} a {end_date.strftime('%d/%m/%Y')}"
        elif start_date:
            label = f"Desde {start_date.strftime('%d/%m/%Y')}"
        else:
            label = f"Ate {end_date.strftime('%d/%m/%Y')}"

        self._set_date_filter(start_date, end_date, label, "custom")
    ...
    def open_date_picker(self):
        ...
        buttons_frame = create_action_footer(main_frame, padx=0, pady=(12, 0))
        ...
        confirm_btn = create_action_button(
            buttons_frame,
            text="Confirmar",
            command=on_confirm,
            role="primary",
    ...
    def refresh_data(self, force: bool = False):
        ...
        def callback(repairs, error):
            ...
            column_weights = [10, 20, 20, 10, 10, 12, 9, 6]
            ...
            for i, header in enumerate(headers):
                header_anchor = "center" if i >= 5 else "w"
                label = ctk.CTkLabel(
                    header_frame,
                    text=header,
                    font=ctk.CTkFont(size=12, weight="bold"),
                ...
                label.grid(row=0, column=i, padx=8, pady=9, sticky="ew")
    ...
    def _show_repair_details_impl(self, repair: dict):
        ...
        popup.configure(fg_color="#151617")
        ...
        title_label.pack(anchor="w", padx=16, pady=(16, 14))
        ...
        scrollable.pack(fill="both", expand=True, padx=16)
        ...
        buttons_frame = create_action_footer(main_frame, pady=(12, 0))
    ...
    def open_edit_repair_window(self, repair_id: int, details_popup=None):
        ...
        def load_repair_data():
            ...
            def callback(repair, error):
                ...
                edit_dialog.configure(fg_color="#151617")
                ...
                components_section = ctk.CTkFrame(
                    scrollable,
                    fg_color=PANEL_BG,
                    corner_radius=8,
                    border_width=1,
                ...
                components_list_container = ctk.CTkScrollableFrame(
                    components_section,
                    height=200,
                    fg_color="#151617",
                    corner_radius=8,
                ...
                # Separador visual
                separator = ctk.CTkFrame(scrollable, fg_color=BORDER, height=1)
                ...
                labor_type_segmented = LaborTypeSelector(
                    scrollable,
                ...
                # Botões de ação
                buttons_frame = create_action_footer(main_frame, padx=16, pady=(10, 16))
                ...
                apply_modern_style(edit_dialog)
    ...
    @staticmethod
    def _safe_invoice_filename_part(value, fallback: str) -> str:
    ...
    @classmethod
    def _build_default_invoice_filename(cls, repair: dict) -> str:
        client_name = cls._safe_invoice_filename_part(repair.get("client"), "Cliente")
        invoice_id = cls._safe_invoice_filename_part(repair.get("id"), "sem_id")
        date_str = str(repair.get("date", "") or "")
        ...
        return f"Fatura_{client_name}_{invoice_id}_{date_formatted}.pdf"
    ...
    def create_repair_row(self, repair: dict):
        ...
        date_label.grid(row=0, column=0, padx=10, pady=10, sticky="ew")
        ...
        client_label.grid(row=0, column=1, padx=10, pady=10, sticky="ew")
        ...
        problem_label.grid(row=0, column=2, padx=10, pady=10, sticky="ew")
        ...
        imei_label.grid(row=0, column=3, padx=10, pady=10, sticky="ew")
        ...
        total_label.grid(row=0, column=4, padx=10, pady=10, sticky="ew")
        ...
        def set_repair_status(new_status):
            if new_status == repair_status:
                return
        ...
        def open_progress_menu():
            existing_menu = getattr(self, "_history_progress_menu", None)
            if existing_menu and existing_menu.winfo_exists():
                existing_owner = getattr(self, "_history_progress_menu_owner", None)
                existing_menu.destroy()
                self._history_progress_menu = None
                if existing_owner is progress_button:
                    self._history_progress_menu_owner = None
                    return

            menu = ctk.CTkToplevel(self.app.root)
            self._history_progress_menu = menu
            self._history_progress_menu_owner = progress_button
            menu.withdraw()
            menu.overrideredirect(True)
            menu.attributes("-topmost", True)
            menu.configure(fg_color=PANEL_BG)
            ...
            def choose_status(value):
                if menu.winfo_exists():
                    menu.destroy()
                self._history_progress_menu = None
                self._history_progress_menu_owner = None
                set_repair_status(value)

            progress_options = ["Em Análise", "Aguardar Peças", "Pronto a Entregar"]
            for index, option in enumerate(progress_options):
                style = repair_status_badge_style(option) if option == repair_status else button_style("secondary", compact=True)
                option_button = ctk.CTkButton(
                    menu_frame,
                    text=option,
                    command=lambda value=option: choose_status(value),
                    anchor="w",
                    width=172,
                    **style,
                )
                ...
                option_button.pack(
                    fill="x",
                    padx=6,
                    pady=(6 if index == 0 else 2, 6 if index == len(progress_options) - 1 else 2),
                )

            menu.update_idletasks()
            x = progress_button.winfo_rootx() - menu.winfo_width() + progress_button.winfo_width()
            y = progress_button.winfo_rooty() + progress_button.winfo_height() + 4
            menu.geometry(f"+{max(0, x)}+{max(0, y)}")
            menu.deiconify()
            menu.focus_force()
            menu.bind("<Escape>", lambda _event: menu.destroy())
        ...
        status_colors = {
            "Em Análise": PRIMARY,
            "Aguardar Peças": WARNING,
            "Pronto a Entregar": SUCCESS
        }
        status_hover_colors = {
            "Em Análise": PRIMARY_HOVER,
            "Aguardar Peças": WARNING_HOVER,
            "Pronto a Entregar": SUCCESS_HOVER
        }
        ...
        status_badge = {
            "Em AnÃ¡lise": ("#10243d", PRIMARY, "#163555"),
            "Aguardar PeÃ§as": ("#34230b", WARNING, "#443012"),
            "Pronto a Entregar": ("#0f2d1d", SUCCESS, "#163d29"),
        }.get(repair_status, ("#10243d", PRIMARY, "#163555"))

        if "Aguardar" in repair_status:
            status_badge = ("#A16207", "#fff7d6", "#B7791F")
        elif "Pronto" in repair_status:
            status_badge = ("#15803D", "#e8fff1", "#16A34A")
        else:
            status_badge = ("#1D4ED8", "#eaf1ff", "#2563EB")

        progress_button = ctk.CTkButton(
            row_frame,
            text=status_text,
            command=open_progress_menu,
            width=132,
            **repair_status_badge_style(repair_status),
        )
        keep_custom_style(progress_button)
        progress_button.grid(row=0, column=5, padx=8, pady=8)
        ...
        payment_badge = (
            ("#15803D", "#e8fff1", "#16A34A")
            if is_paid
            else ("#B91C1C", "#fff1f1", "#DC2626")
        )
        status_button = ctk.CTkButton(
            ...
            width=98,
            **payment_status_badge_style(is_paid),
        )
        keep_custom_style(status_button)
        status_button.grid(row=0, column=6, padx=8, pady=8)

        # Ações (Detalhes, PDF, Remover) - COMPACTOS E UNIFORMES
        actions_frame = ctk.CTkFrame(row_frame, fg_color="transparent")
        actions_frame.grid(row=0, column=7, padx=8, pady=8)

        # Botão Detalhes - COMPACTO E UNIFORME
        # Botão PDF (Gerar Fatura)
        def generate_pdf():
            # Formatar nome do ficheiro
            default_filename = self._build_default_invoice_filename(repair)
        ...
        def open_actions_menu():
            existing_menu = getattr(self, "_history_action_menu", None)
            if existing_menu and existing_menu.winfo_exists():
                existing_owner = getattr(self, "_history_action_menu_owner", None)
                existing_menu.destroy()
                self._history_action_menu = None
                if existing_owner is action_button_menu:
                    self._history_action_menu_owner = None
                    return

            menu = ctk.CTkToplevel(self.app.root)
            self._history_action_menu = menu
            self._history_action_menu_owner = action_button_menu
            menu.withdraw()
            menu.overrideredirect(True)
            menu.attributes("-topmost", True)
            menu.configure(fg_color=PANEL_BG)
            ...
            action_items = [
                ("Detalhes", lambda: self.show_repair_details(repair), "secondary"),
                ("PDF", generate_pdf, "secondary"),
                ("Apagar", safe_delete, "danger"),
            ]

            for index, (label, action, role) in enumerate(action_items):
                action_item_button = ctk.CTkButton(
                    menu_frame,
                    text=label,
                    command=lambda a=action: run_action(a),
                    anchor="w",
                    width=140,
                    **button_style(role, compact=True),
                )
                action_item_button._indutech_keep_style = True
                action_item_button.pack(
                    fill="x",
                    padx=6,
                    pady=(6 if index == 0 else 2, 6 if index == len(action_items) - 1 else 2),
                )

            menu.update_idletasks()
            x = action_button_menu.winfo_rootx() - menu.winfo_width() + action_button_menu.winfo_width()
            y = action_button_menu.winfo_rooty() + action_button_menu.winfo_height() + 4
            menu.geometry(f"+{max(0, x)}+{max(0, y)}")
            menu.deiconify()
            menu.focus_force()
            menu.bind("<Escape>", lambda _event: menu.destroy())

        action_button_menu = ctk.CTkButton(
            actions_frame,
            text="⋯",
            command=open_actions_menu,
            **icon_button_style(),
        )
        keep_custom_style(action_button_menu)
        action_button_menu.configure(text="...")
        action_button_menu.grid(row=0, column=0, padx=0, pady=0)
